import type { RiviereGraph, EventComponent, EventHandlerComponent } from '@living-architecture/riviere-schema'
import type { PublishedEvent, EventSubscriber, EventHandlerInfo, KnownSourceEvent, UnknownSourceEvent } from './event-types'
...
export function queryPublishedEvents(graph: RiviereGraph, domainName?: string): PublishedEvent[] {
  ...
  const handlers = graph.components.filter((c): c is EventHandlerComponent => c.type === 'EventHandler')
...
function buildEventNameMap(graph: RiviereGraph): Map<string, EventComponent> {
  return new Map(graph.components.filter((c): c is EventComponent => c.type === 'Event').map((e) => [e.eventName, e]))
}
...
function buildEventHandlerInfo(handler: EventHandlerComponent, eventByName: Map<string, EventComponent>): EventHandlerInfo {
