...
describe('nodeDepths', () => {
  ...
  it('returns depth based on hops from entry point', () => {
    ...
    graph.components.push(
      createAPIComponent({ id: 'test:mod:api:users', name: 'Users API', domain: 'test' }),
      createUseCaseComponent({ id: 'test:mod:uc:getUser', name: 'Get User', domain: 'test' }),
    )
    graph.links = [
      { source: 'test:mod:ui:page', target: 'test:mod:api:users' },
      { source: 'test:mod:api:users', target: 'test:mod:uc:getUser' },
    ]
  ...
  it('returns minimum depth when reachable from multiple entry points', () => {
    const graph = createMinimalValidGraph()
    graph.components.push(
      createAPIComponent({ id: 'test:mod:api:direct', name: 'Direct API', domain: 'test' }),
      createUseCaseComponent({ id: 'test:mod:uc:shared', name: 'Shared UC', domain: 'test' }),
    )
    graph.links = [
      { source: 'test:mod:ui:page', target: 'test:mod:uc:shared' },
      { source: 'test:mod:api:direct', target: 'test:mod:uc:shared' },
    ]
  ...
  it('excludes unreachable nodes from result', () => {
    const graph = createMinimalValidGraph()
    graph.components.push(
      createUseCaseComponent({ id: 'test:mod:uc:orphan', name: 'Orphan', domain: 'test' }),
    )
  ...
  it('handles source with multiple outgoing links', () => {
    const graph = createMinimalValidGraph()
    graph.components.push(
      createUseCaseComponent({ id: 'test:mod:uc:a', name: 'UC A', domain: 'test' }),
      createUseCaseComponent({ id: 'test:mod:uc:b', name: 'UC B', domain: 'test' }),
    )
    graph.links = [
      { source: 'test:mod:ui:page', target: 'test:mod:uc:a' },
      { source: 'test:mod:ui:page', target: 'test:mod:uc:b' },
    ]
  ...
  it('returns empty map for graph with no entry points', () => {
    const graph: RiviereGraph = {
      ...
      components: [
        { id: 'test:mod:uc:a', type: 'UseCase', name: 'A', domain: 'test', module: 'mod', sourceLocation: defaultSourceLocation },
        { id: 'test:mod:uc:b', type: 'UseCase', name: 'B', domain: 'test', module: 'mod', sourceLocation: defaultSourceLocation },
      ],
      links: [{ source: 'test:mod:uc:a', target: 'test:mod:uc:b' }],
    }
