...
describe('ComparisonPage', () => {
  ...
  describe('comparison results', () => {
    ...
    it('displays stats showing added/removed/modified/unchanged counts', async () => {
      const domainMetadata = {
        d: { description: 'Test domain', systemType: 'domain' },
      }
      const sl = { repository: 'test-repo', filePath: 'src/test.ts' }
      const beforeJson = JSON.stringify({
        version: '1.0',
        metadata: { name: 'Before', domains: domainMetadata },
        components: [
          { id: 'd:m:api:unchanged', type: 'API', apiType: 'REST', httpMethod: 'GET', path: '/unchanged', name: 'Same', domain: 'd', module: 'm', sourceLocation: sl },
          { id: 'd:m:api:removed', type: 'API', apiType: 'REST', httpMethod: 'GET', path: '/removed', name: 'Gone', domain: 'd', module: 'm', sourceLocation: sl },
        ],
        links: [],
      })
      const afterJson = JSON.stringify({
        version: '1.0',
        metadata: { name: 'After', domains: domainMetadata },
        components: [
          { id: 'd:m:api:unchanged', type: 'API', apiType: 'REST', httpMethod: 'GET', path: '/unchanged', name: 'Same', domain: 'd', module: 'm', sourceLocation: sl },
          { id: 'd:m:api:added', type: 'API', apiType: 'REST', httpMethod: 'GET', path: '/added', name: 'New', domain: 'd', module: 'm', sourceLocation: sl },
        ],
        links: [],
      })
      stubFileReaderWithMultipleContents([beforeJson, afterJson])
  ...
  describe('detailed changes list', () => {
    afterEach(() => {
      vi.unstubAllGlobals()
    })

    function createComparisonGraphs(): { beforeJson: string; afterJson: string } {
      const domainMetadata = {
      ...
      const beforeJson = JSON.stringify({
        ...
        metadata: { name: 'Before', domains: domainMetadata },
        components: [
          { id: 'orders:checkout:api:unchanged', type: 'API', apiType: 'REST', httpMethod: 'GET', path: '/orders', name: 'GET /orders', domain: 'orders', module: 'checkout', sourceLocation: sl },
          { id: 'orders:checkout:api:removed', type: 'API', apiType: 'REST', httpMethod: 'DELETE', path: '/orders/:id', name: 'DELETE /orders/:id', domain: 'orders', module: 'checkout', sourceLocation: sl },
          { id: 'orders:checkout:api:modified', type: 'API', apiType: 'REST', httpMethod: 'POST', path: '/orders', name: 'POST /orders', domain: 'orders', module: 'checkout', description: 'Old description', sourceLocation: sl },
        ],
        links: [],
      })
      const afterJson = JSON.stringify({
        ...
        components: [
          { id: 'orders:checkout:api:unchanged', type: 'API', apiType: 'REST', httpMethod: 'GET', path: '/orders', name: 'GET /orders', domain: 'orders', module: 'checkout', sourceLocation: sl },
          { id: 'orders:checkout:api:added', type: 'API', apiType: 'REST', httpMethod: 'POST', path: '/orders/:id/refund', name: 'POST /orders/:id/refund', domain: 'orders', module: 'checkout', sourceLocation: sl },
          { id: 'orders:checkout:api:modified', type: 'API', apiType: 'REST', httpMethod: 'POST', path: '/orders', name: 'POST /orders', domain: 'orders', module: 'checkout', description: 'New description', sourceLocation: sl },
        ],
        links: [],
      })
    ...
    function createComparisonGraphsWithMultipleDomains(): { beforeJson: string; afterJson: string } {
      const domainMetadata = {
        orders: { description: 'Order domain', systemType: 'domain' },
      ...
      const afterJson = JSON.stringify({
        ...
        components: [
          { id: 'orders:checkout:api:unchanged', type: 'API', apiType: 'REST', httpMethod: 'GET', path: '/orders', name: 'GET /orders', domain: 'orders', module: 'checkout', sourceLocation: sl },
          { id: 'orders:checkout:api:added', type: 'API', apiType: 'REST', httpMethod: 'POST', path: '/orders/:id/refund', name: 'POST /orders/:id/refund', domain: 'orders', module: 'checkout', sourceLocation: sl },
          { id: 'shipping:fulfillment:usecase:added', type: 'UseCase', name: 'Create Shipment', domain: 'shipping', module: 'fulfillment', sourceLocation: sl },
        ],
        links: [],
      })
      return { beforeJson, afterJson }
    ...
    function createComparisonGraphsWithMultipleTypes(): { beforeJson: string; afterJson: string } {
      const domainMetadata = {
        orders: { description: 'Order domain', systemType: 'domain' },
      }
      const sl = { repository: 'test-repo', filePath: 'src/test.ts' }
      ...
      const afterJson = JSON.stringify({
        ...
        components: [
          { id: 'orders:checkout:api:unchanged', type: 'API', apiType: 'REST', httpMethod: 'GET', path: '/orders', name: 'GET /orders', domain: 'orders', module: 'checkout', sourceLocation: sl },
          { id: 'orders:checkout:api:added', type: 'API', apiType: 'REST', httpMethod: 'POST', path: '/orders/:id/refund', name: 'POST /orders/:id/refund', domain: 'orders', module: 'checkout', sourceLocation: sl },
          { id: 'orders:checkout:usecase:added', type: 'UseCase', name: 'Process Refund', domain: 'orders', module: 'checkout', sourceLocation: sl },
        ],
        links: [],
      })
      return { beforeJson, afterJson }
    ...
    async function setupComparisonAndCompare(user: ReturnType<typeof userEvent.setup>, beforeJson: string, afterJson: string): Promise<void> {
      const beforeInput = screen.getByLabelText(/upload before file/i)
      const afterInput = screen.getByLabelText(/upload after file/i)
    ...
    it('shows detailed changes section after comparison', async () => {
      const { beforeJson, afterJson } = createComparisonGraphs()
      stubFileReaderWithMultipleContents([beforeJson, afterJson])
    ...
    it('shows domain filter options based on changed nodes', async () => {
      const { beforeJson, afterJson } = createComparisonGraphsWithMultipleDomains()
      stubFileReaderWithMultipleContents([beforeJson, afterJson])
    ...
    it('shows node type filter options based on changed nodes', async () => {
      const { beforeJson, afterJson } = createComparisonGraphsWithMultipleTypes()
      stubFileReaderWithMultipleContents([beforeJson, afterJson])
  ...
  describe('domain connection diff', () => {
    ...
    function createGraphsWithCrossDomainEdges(): { beforeJson: string; afterJson: string } {
      const domainMetadata = {
        orders: { description: 'Order domain', systemType: 'domain' },
        payments: { description: 'Payment domain', systemType: 'domain' },
      }
      const sl = { repository: 'test-repo', filePath: 'src/test.ts' }
      const beforeJson = JSON.stringify({
        version: '1.0',
        metadata: { name: 'Before', domains: domainMetadata },
        components: [
          { id: 'orders:checkout:api:place-order', type: 'API', apiType: 'REST', httpMethod: 'POST', path: '/orders', name: 'POST /orders', domain: 'orders', module: 'checkout', sourceLocation: sl },
          { id: 'payments:billing:usecase:process-payment', type: 'UseCase', name: 'Process Payment', domain: 'payments', module: 'billing', sourceLocation: sl },
        ],
        links: [],
      })
      const afterJson = JSON.stringify({
        ...
        links: [
          { source: 'orders:checkout:api:place-order', target: 'payments:billing:usecase:process-payment', type: 'sync' },
        ],
      })
      return { beforeJson, afterJson }
    }
    ...
    it('shows domain connection diff visualization after comparison', async () => {
      const { beforeJson, afterJson } = createGraphsWithCrossDomainEdges()
      stubFileReaderWithMultipleContents([beforeJson, afterJson])
